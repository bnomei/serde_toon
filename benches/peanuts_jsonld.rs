...
fn bench_peanuts_jsonld(c: &mut Criterion) {
    ...
    group.bench_function("peanuts_jsonld/encode_toon", |b| {
        b.iter(|| {
            let encoded = serde_toon::to_string(black_box(&dataset)).expect("encode failed");
            black_box(encoded);
        });
    });
    group.bench_function("peanuts_jsonld/decode_toon", |b| {
        b.iter(|| {
    ...
    group.bench_function("peanuts_jsonld/encode_json", |b| {
        b.iter(|| {
            let encoded = serde_json::to_string(black_box(&dataset)).expect("json encode failed");
            black_box(encoded);
        });
    });
    group.bench_function("peanuts_jsonld/decode_json", |b| {
        b.iter(|| {
            let decoded: JsonLdDataset =
