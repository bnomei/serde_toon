...
fn parse_args() -> (u64, PathBuf, u32) {
    let mut seconds = 30_u64;
    let mut out_prefix = PathBuf::from("profiles/decode_github");
    ...
    (seconds, out_prefix, frequency)
}

fn main() {
    let (seconds, out_prefix, frequency) = parse_args();
    let repos: Vec<GitHubRepo> = serde_json::from_slice(GITHUB_REPOS_JSON)
