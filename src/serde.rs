...
pub(crate) fn to_value<T: Serialize>(value: &T) -> ToonResult<crate::types::JsonValue> {
    ser::to_value(value).map_err(|err| ToonError::SerializationError(err.to_string()))
}

pub(crate) fn from_value<T: DeserializeOwned>(value: &crate::types::JsonValue) -> ToonResult<T> {
    de::from_value(value).map_err(|err| ToonError::DeserializationError(err.to_string()))
}
...
/// Serialize a `serde_json::Value` to a TOON string using default options.
pub fn to_string_value(value: &Value) -> ToonResult<String> {
    crate::encode::encode_value(value, &EncodeOptions::default())
}
...
/// # Examples
/// ```
/// use serde_toon::{to_string_with_options, Delimiter, EncodeOptions};
...
/// Serialize a `serde_json::Value` to a TOON string using custom options.
pub fn to_string_value_with_options(
    value: &Value,
    opts: &EncodeOptions,
) -> ToonResult<String> {
    crate::encode::encode_value(value, opts)
}
...
/// Serialize a `serde_json::Value` to a UTF-8 byte vector using default options.
pub fn to_vec_value(value: &Value) -> ToonResult<Vec<u8>> {
    crate::encode::encode_value_to_vec(value, &EncodeOptions::default())
}

/// Serialize a value to a UTF-8 byte vector using custom options.
///
/// # Examples
/// ```
/// use serde_toon::{to_vec_with_options, Delimiter, EncodeOptions};
/// let opts = EncodeOptions::new().with_delimiter(Delimiter::Pipe);
/// let bytes = to_vec_with_options(&serde_json::json!({"a": 1}), &opts)?;
/// assert!(!bytes.is_empty());
/// # Ok::<(), serde_toon::ToonError>(())
/// ```
pub fn to_vec_with_options<T: Serialize>(
    ...
    crate::encode::encode_to_vec(value, opts)
}

/// Serialize a `serde_json::Value` to a UTF-8 byte vector using custom options.
pub fn to_vec_value_with_options(value: &Value, opts: &EncodeOptions) -> ToonResult<Vec<u8>> {
    crate::encode::encode_value_to_vec(value, opts)
}
...
/// Serialize a `serde_json::Value` to a writer using default options.
pub fn to_writer_value<W: Write>(mut writer: W, value: &Value) -> ToonResult<()> {
...
/// # Examples
/// ```
/// use serde_toon::{to_writer_with_options, Delimiter, EncodeOptions};
/// let opts = EncodeOptions::new().with_delimiter(Delimiter::Pipe);
/// let mut buffer = Vec::new();
...
pub fn to_writer_with_options<T: Serialize, W: Write>(
    ...
    let encoded = to_vec_with_options(value, opts)?;
    writer
        .write_all(&encoded)
        .map_err(|err| ToonError::InvalidInput(format!("Failed to write output: {err}")))
}

/// Serialize a `serde_json::Value` to a writer using custom options.
pub fn to_writer_value_with_options<W: Write>(
    mut writer: W,
    value: &Value,
    opts: &EncodeOptions,
) -> ToonResult<()> {
    let encoded = to_vec_value_with_options(value, opts)?;
    writer
        .write_all(&encoded)
        .map_err(|err| ToonError::InvalidInput(format!("Failed to write output: {err}")))
}
