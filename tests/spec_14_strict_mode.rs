...
fn spec14_strict_mode_encode(
    ...
    match expected {
        Some(expected) => {
            let actual = Spec14Adapter::encode(&input, &options)
                .unwrap_or_else(|err| panic!("encode failed: {err}"));
            assert_eq!(actual, expected);
        }
        None => {
            assert!(Spec14Adapter::encode(&input, &options).is_err());
        }
    }
}
...
fn spec14_strict_mode_decode(
    ...
    match expected {
        Some(expected) => {
            let actual = Spec14Adapter::decode(input, &options)
                .unwrap_or_else(|err| panic!("decode failed: {err}"));
            assert_eq!(actual, expected);
        }
        None => {
            assert!(Spec14Adapter::decode(input, &options).is_err());
        }
    }
}
...
#[case("a:\n\tb: 1", false)]
#[case("items[2]:\n  - 1\n\n  - 2", false)]
fn spec14_strict_mode_validate(#[case] input: &str, #[case] valid: bool) {
    let result = Spec14Adapter::validate(input);
    if valid {
        assert!(result.is_ok());
    } else {
