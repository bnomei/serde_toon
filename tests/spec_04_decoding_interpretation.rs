...
fn spec04_decoding_interpretation_encode(
    #[case] input: Value,
    ...
    match expected {
        Some(expected) => {
            let actual = Spec04Adapter::encode(&input, &options)
                .unwrap_or_else(|err| panic!("encode failed: {err}"));
            assert_eq!(actual, expected);
        }
        None => {
            assert!(Spec04Adapter::encode(&input, &options).is_err());
        }
    }
}
...
#[case("a: \"true\"", Some(json!({"a": "true"})), SpecOptions::default())]
#[case("a: \"123\"", Some(json!({"a": "123"})), SpecOptions::default())]
#[case(
    "a: true\nb: false\nc: null",
...
#[case("n: -0001", Some(json!({"n": "-0001"})), SpecOptions::default())]
...
fn spec04_decoding_interpretation_decode(
    ...
    match expected {
        Some(expected) => {
            let actual = Spec04Adapter::decode(input, &options)
                .unwrap_or_else(|err| panic!("decode failed: {err}"));
            assert_eq!(actual, expected);
        }
        None => {
            assert!(Spec04Adapter::decode(input, &options).is_err());
        }
    }
}
