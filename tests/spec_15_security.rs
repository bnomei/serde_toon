...
#[case(json!({"v": "a,b"}), Some("v: \"a,b\""), SpecOptions::default())]
#[case(json!({"v": "a|b"}), Some("v: \"a|b\""), SpecOptions::default().with_delimiter('|'))]
#[case(json!({"v": "emoji: 😄"}), Some("v: \"emoji: 😄\""), SpecOptions::default())]
fn spec15_security_encode(
    #[case] input: Value,
    ...
    match expected {
        Some(expected) => {
            let actual = Spec15Adapter::encode(&input, &options)
                .unwrap_or_else(|err| panic!("encode failed: {err}"));
            assert_eq!(actual, expected);
        }
        None => {
            assert!(Spec15Adapter::encode(&input, &options).is_err());
        }
    }
}
...
fn spec15_security_decode(
    ...
    match expected {
        Some(expected) => {
            let actual = Spec15Adapter::decode(input, &options)
                .unwrap_or_else(|err| panic!("decode failed: {err}"));
            assert_eq!(actual, expected);
        }
        None => {
            assert!(Spec15Adapter::decode(input, &options).is_err());
        }
    }
}
...
#[case("emoji: 😄", true)]
fn spec15_security_validate(#[case] input: &str, #[case] valid: bool) {
    let result = Spec15Adapter::validate(input);
    if valid {
        assert!(result.is_ok());
    } else {
