...
fn fixture_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("spec/tests/fixtures")
}
...
fn load_fixture_dir(category: &str) -> Vec<(PathBuf, FixtureFile)> {
        ...
        .unwrap_or_else(|err| panic!("failed to read fixture dir {}: {err}", root.display()))
    {
        let entry = entry.unwrap_or_else(|err| {
            panic!("failed to read fixture dir entry {}: {err}", root.display())
        });
        let path = entry.path();
