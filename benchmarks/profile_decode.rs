...
fn parse_args() -> (u64, PathBuf, u32, Option<PathBuf>) {
    let mut seconds = 30_u64;
    let mut out_prefix = PathBuf::from("benchmarks/profiles/decode_github");
    ...
    (seconds, out_prefix, frequency, input)
}

fn main() {
    let (seconds, out_prefix, frequency, input) = parse_args();
    ...
    let toon = if let Some(repos) = &repos {
    ...
    while start.elapsed() < deadline {
        let input = std::hint::black_box(toon.as_str());
        if repos.is_some() {
            let decoded: Vec<GitHubRepo> = serde_toon::from_str(input).expect("decode failed");
            std::hint::black_box(decoded);
        } else {
            let decoded: Value = serde_toon::from_str(input).expect("decode failed");
            std::hint::black_box(decoded);
        }
        iterations += 1;
    }
